...
const DocumentUpload: React.FC<DocumentUploadProps> = ({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const uploadPromises = selectedFiles.map(async ({ file, category }, index) => {
        console.log(`[${index + 1}/${selectedFiles.length}] Uploading: ${file.name} to case ${caseId}`);
        ...
        // Create secure file path for case documents
        const timestamp = Date.now();
        const randomId = Math.random().toString(36).substring(2, 8);
        const safeName = file.name.replace(/[^a-zA-Z0-9.-]/g, '_');
        const fileName = `cases/${caseId}/${timestamp}-${randomId}-${safeName}`;
      ...
      toast({
        title: "Sucesso",
        description: `${results.length} documento(s) enviado(s) para o caso com sucesso!`,
        variant: "success"
      });
