...
-- Apply third: Core legal compliance functionality
...
-- 2. Deadline Notifications Table - Automated alert scheduling  
CREATE TABLE IF NOT EXISTS deadline_notifications (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CONSTRAINT fk_deadline_notifications_deadline FOREIGN KEY (deadline_id) REFERENCES case_deadlines(id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_deadline_notifications_scheduled ON deadline_notifications(notification_date, status);
...
CREATE POLICY "Staff can view case deadlines for assigned cases" ON case_deadlines
    FOR SELECT USING (
        EXISTS (
            ...
            WHERE c.id = case_deadlines.case_id AND au.user_id = auth.uid() AND sca.is_active = true
        )
...
CREATE POLICY "Staff can view court integrations for assigned cases" ON court_integrations
    FOR SELECT USING (
        EXISTS (
            ...
            JOIN cases c ON c.client_id = sca.client_id
            JOIN admin_users au ON au.staff_id::uuid = sca.staff_id
            WHERE c.id = court_integrations.case_id AND au.user_id = auth.uid() AND sca.is_active = true
        )
        OR EXISTS (SELECT 1 FROM admin_users WHERE user_id = auth.uid())
    );
...
('2025-02-13', 'Carnaval', FALSE, TRUE, 'SP'),
('2025-02-14', 'Carnaval', FALSE, TRUE, 'SP'),
('2025-04-18', 'Sexta-feira Santa', FALSE, TRUE, 'SP'),
('2025-05-01', 'Dia do Trabalhador', FALSE, TRUE, 'SP'),
('2025-06-19', 'Corpus Christi', FALSE, TRUE, 'SP'),
('2025-09-07', 'Independência do Brasil', FALSE, TRUE, 'SP'),
('2025-10-12', 'Nossa Senhora Aparecida', FALSE, TRUE, 'SP')
ON CONFLICT (holiday_date, state_code, city_code) DO NOTHING;

-- Insert sample legal templates for Brazilian legal practice
INSERT INTO legal_templates (name, template_type, practice_area, court_type, content, variables, is_active, version, created_at) VALUES
(
...
OAB/{{ESTADO}} {{NUMERO_OAB}}',
    '{
        "CIDADE": "text",
        "NOME_AUTOR": "text", 
        "QUALIFICACAO_AUTOR": "text",
        "NOME_REU": "text",
        "QUALIFICACAO_REU": "text",
        "RELATO_FATOS": "textarea",
        "FUNDAMENTACAO_JURIDICA": "textarea", 
        "PEDIDOS": "textarea",
...
OAB/{{ESTADO}} {{NUMERO_OAB}}',
    '{
        ...
        "TIPO_ACAO": "text",
        "NOME_AUTOR": "text",
        "PRELIMINARES": "textarea",
        "MERITO": "textarea",
        "PEDIDOS_ADICIONAIS": "textarea",
        "CIDADE": "text",
        "DATA": "date",
        "NOME_ADVOGADO": "text",
        "ESTADO": "text",
...
-- Insert sample case workflow phases for different practice areas
...
-- Insert sample OAB compliance rules
...
COMMENT ON TABLE deadline_notifications IS 'Automated alert scheduling for legal deadlines';
